...
local discordFrame = { x = 920, y = 621, w = 775, h = 580 }
local slackFrame = { x = 850, y = 621, w = 700, h = 580 }
...
local function absFrame(frame)
    ...
    if not s then return frame end
    local sf = s:frame()
...
local function enforceFrame(win, frame)
    ...
    for i = 0, 3 do
        hs.timer.doAfter(0.2 * i, function()
            if win:isStandard() then win:setFrame(f) end
        end)
    end
end
